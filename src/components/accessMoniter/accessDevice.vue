    ...
    <div class="contantHeader">
      ...
      >
        ...
        <div class="deviceList">
          ...
          >
            <el-tab-pane
              :label="$t('accessSystem.shexiangji')"
              name="1"
            ></el-tab-pane>
            <el-tab-pane
              :label="$t('accessSystem.zhinengyingjian')"
              name="3"
            ></el-tab-pane>
            <!-- <el-tab-pane
              label="智能硬件"
          ...
          >
            <el-table-column
              :label="$t('accessSystem.mingcheng')"
              width="180"
              prop="name"
            >
            </el-table-column>
...
export default {
  ...
  methods: {
      ...
      const promise = new Promise((resolve, reject) => {
        this.$http
          .findAllDevice({
              ...
              groupNo: this.$store.state.sccessDevice.accessManage.groupData.groupNo,
              ...
              if (res.data.data.hasOwnProperty(key)) {
                ...
                for (const key2 in this.$store.state.sccessDevice.accessManage.deviceList) {
                  if (
                    ...
                    const device = this.$store.state.sccessDevice.accessManage.deviceList[
                      key2
                    ];
    ...
    // 添加设备对话框关闭前处理
    addClose() {
      (this.$store.state.sccessDevice.accessManage.addList = []), (this.dialogTableVisible = false);
    },
    //消防开门
    openAll() {
    ...
    settingSubmit() {
      ...
      this.$http
        .post(updateDevice, this.settingForm)
        .then(res => {
          if (res.data.status == 200) {
    ...
    insertDevice() {
      let paramsId = []
      console.log(this.deviceAlreadyList, this.$store.state.sccessDevice.accessManage.addList)
      for (var key in this.$store.state.sccessDevice.accessManage.addList) {
        if (this.$store.state.sccessDevice.accessManage.addList.hasOwnProperty(key)) {
          const element = this.$store.state.sccessDevice.accessManage.addList[key];
          paramsId.push(element.id);
          for (const key2 in this.$store.state.sccessDevice.accessManage.deviceAlreadyList) {
            if (this.$store.state.sccessDevice.accessManage.deviceAlreadyList.hasOwnProperty(key2)) {
              const element2 = this.$store.state.sccessDevice.accessManage.deviceAlreadyList[key2];
    ...
    deleteTag(tag) {
      console.log(tag);
      this.$store.state.sccessDevice.accessManage.addList.splice(this.$store.state.sccessDevice.accessManage.addList.indexOf(tag), 1);
    ...
    handleEdit(index, row) {
      ...
      this.$store.state.sccessDevice.accessManage.addList.push({ name: row.name, id: row.id, closable: true });
    },
    // 新增设备列表查询
    getDevices() {
