<template>
  <div>
    <el-dialog
      ...
      :title="
        isEditPeo === 0
          ...
          : isEditPeo === 1
          ? $t('peopleManageList.editPeople')
          : $t('peopleManageList.personDetails')
      "
    ...
    >
      ...
      >
        <div
          v-for="(item, index) in isShow.tableDataLabel"
          v-if="item.property != 'id'"
          :key="item.code"
        >
          <el-form-item
            :label="item.name"
            v-if="item.name === $t('peopleManageList.photo')"
          ...
          >
            <el-input
              ...
              :placeholder="
                language === 'en'
                  ? `${$t('peopleManageList.pleaseEnter')} ${item.name}`
                  : `${$t('peopleManageList.pleaseEnter')}${item.name}`
              "
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="submit" :disabled="isEditPeo === 2">{{
          $t("common.ok")
        }}</el-button>
      </span>
    </el-dialog>
...
export default {
        ...
        if (val.show) {
          ...
          if (val.show && this.isEditPeo !== 0) {
            ...
            this.$nextTick(() => {
              ...
              if (
                this.FormData.hasOwnProperty("pictures") &&
                this.isEditPeo === 0
              ) {
  ...
  methods: {
    ...
    sunk(param) {
      ...
      xhr.open("post", "/api/v2/feature/use-binary-do-feature", true);
      form.append("picture", param.file);
      form.append("token", '7d1e52d3cf0142e19b5901eb1ef91372');
      if(this.specialValue){
        form.append("snacks", this.specialValue);
      }
      var self = this
      xhr.onload = function() {
        var response = JSON.parse(xhr.response);
        self.specialValue = response.data
      };
      xhr.send(form);
    },
    ...
    submit() {
      ...
      if (this.isEditPeo === 1) {
        ...
        this.FormData.snacks= this.specialValue;
        ...
        this.$http.personModify(this.FormData).then(
          res => {
        ...
        console.log(this.FormData)
         this.FormData.snacks= this.specialValue;
        this.$http.personInsert(this.FormData).then(
          res => {
            if (res.data.status == 200) {
              this.$message.success(res.data.message);
              this.$emit("active", "refresh");
    ...
    rtxForm() {
      ...
      this.isShow.tableDataLabel.forEach(item => {
        if (
          item.name === this.$t("peopleManageList.photo") &&
          ...
          this.$message.warning(
            `${item.name} ${this.$t("peopleManageList.notEmpty")}`
          );
          isTrue = false;
        }
      });
    ...
    initSetForm() {
      ...
      this.FormData.pictures = JSON.parse(
        JSON.stringify(this.selectInfo.pictures)
      );
