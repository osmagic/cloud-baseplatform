  ...
  <div id="deviceList">
    <div class="deviceControl-Page">
      ...
      <el-dialog
        ...
        width="575px"
        :before-close="cancelcreatedialogVisible"
      >
        <el-form
          ref="createform"
          :model="createform"
        ...
        >
          <el-form-item
            :label="$t('deviceControl.groupName') + '：'"
            prop="groupName"
            :label-width="$i18n.locale == 'zh' ? '100px' : '108px'"
          >
            <el-input
              v-model="createform.groupName"
              :placeholder="$t('deviceControl.groupName')"
            ></el-input>
          </el-form-item>
          <el-form-item
            :label="$t('deviceControl.groupDescription') + '：'"
            prop="description"
            :label-width="$i18n.locale == 'zh' ? '100px' : '141px'"
          >
            <el-input
              v-model="createform.description"
              :placeholder="$t('deviceControl.groupDescription')"
            ></el-input>
          </el-form-item>
        ...
        <el-form-item :label="$t('deviceList.latitude') + '：'" v-if="showLongtitude" prop="lat">
          ...
          <el-form-item class="sub">
            <el-button type="primary" @click="cancelcreatedialogVisible">{{
              $t("common.cancel")
            }}</el-button>
      ...
      <!-- 编辑设备组模态框 -->
      <el-dialog
        :title="$t('deviceControl.editGroup')"
        :visible.sync="updatedialogVisible"
        width="575px"
        :before-close="cancelupdatedialogVisible"
      >
        <el-form
          ref="updateform"
          :model="updateform"
        ...
        <el-form-item :label="$t('deviceList.latitude') + '：'" prop="lat" v-if="updateLat">
          ...
          <el-form-item class="sub">
            <el-button type="primary" @click="cancelupdatedialogVisible">{{
              $t("common.cancel")
            }}</el-button>
      ...
      <!-- 删除提示模态框 -->
      <el-dialog
        :title="$t('deviceControl.deleteGroup')"
      ...
      >
        <div class="deleteTop">
          <i class="el-icon-warning"></i>{{ $t("deviceControl.cantDelete") }}
        </div>
        <div class="deleteBottom">
          <div class="deletelook">
            <span @click="findgroupinner">{{
              $t("deviceControl.viewGroup")
            }}</span>
          </div>
          <div class="deleteinfoconfirm">
            <el-button
              type="primary"
              @click="canceldeletedialogVisible"
              size="small"
      ...
      <!-- 查看组内设备模态框 -->
      <el-dialog
        :title="$t('deviceControl.inGroup')"
        :visible.sync="innerdialogVisible"
        width="575px"
        :before-close="cancelinnerdialogVisible"
      >
        <el-table :data="deletegroupNoDevices" height="280px">
        ...
        <div class="sub" style="margin-top: 15px;">
          <el-button
            type="primary"
            @click="cancelinnerdialogVisible"
            size="small"
            >{{ $t("common.ok") }}</el-button
          >
        </div>
  ...
  methods: {
    ...
    querySearchAsync(queryString, cb) {
      let promise = new Promise((resolve, reject) => {
        // 按输入的值查询分组
        this.$http.getGroupList({ groupName: queryString }).then(res => {
      ...
      promise.then(res => {
        ...
        let results = queryString
          ? sugessQuery.filter(this.createStateFilter(queryString))
          : res;
    ...
    createForm() {
      ...
      if (this.showLongtitude == false) {
        ...
        this.$http.deviceGroupInsert(this.createform).then(
          res => {
            if (res.data.status == 200) {
        ...
        // delete this.createform.parentNo;
        // 第一次请求
        this.$http.deviceGroupInsert(this.createform).then(res => {
          if (res.data.status == 200) {
            console.log(res.data.data);
            // 第二次请求时用到的设备组编号
            newCreateform.deviceGroupNo = res.data.data.groupNo;
            // 第二次请求时用到的时间
    ...
    openupdateform() {
      this.$http
        .deviceGroupDetail({
          groupNo: this.updateform.groupNo
        })
        .then(res => {
          if (res.data.status == 200) {
            console.log(res.data.data);
            if (res.data.data.classify) {
              this.updateform.classify = res.data.data.classify;
            }
            if (res.data.data.lat) {
              this.updateform.lat = parseFloat(res.data.data.lat);
            }
            if (res.data.data.lng) {
              this.updateform.lng = parseFloat(res.data.data.lng);
            }
            this.updateAndDeleteId = res.data.data.id;
            this.updateClassify = res.data.data.classify;
            this.updateLng = res.data.data.lng;
            this.updateLat = res.data.data.lat;
    ...
    updateForm() {
      ...
      if (false) {
        ...
        this.$http.deviceGroupModify(this.updateform).then(res => {
          if (res.body.status == 200) {
              ...
              res => {
                if (res.body.status == 200) {
                  this.$message.success(
                    this.$t("deviceControl.modifiedSuccess")
                  );
                  this.$refs.treemenu.initData();
                } else {
                  this.$message.warning(
                    this.$t("common.updateFailed") + res.body.message
                  );
                }
              },
              err => {
        ...
        delete this.updateform.lat;
        delete this.updateform.createDate;
        this.$http.deviceGroupModify(this.updateform).then(
          res => {
            if (res.data.status == 200) {
    ...
    opendeleteform() {
      this.$confirm(
        this.$t("deviceControl.needTodelete"),
        ...
        .then(() => {
          ...
          Promise.all(promises).then(res=>{
             let canDelete = true;
              ...
              if(canDelete){
                 this.$http.deviceGroupDelete( [{groupNo: this.deletegroupNo}]).then(res=>{
                   if(res.data.status==200){
